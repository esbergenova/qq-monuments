          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}>
            {sent ? (
              <div className="glass rounded-xl p-10 text-center">
                ...
                <p className="text-muted-foreground">{t('contact.thankYouText')}</p>
              ...
              <form onSubmit={handleSubmit} className="glass rounded-xl p-8 space-y-5">
                <div>
                  ...
                  <Input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} className="bg-muted/30 border-border/50 focus:border-primary/50" placeholder="John Doe" />
                </div>
                <div>
                  <label className="text-xs font-mono text-muted-foreground mb-1.5 block">{t('contact.email')}</label>
                  <Input type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} className="bg-muted/30 border-border/50 focus:border-primary/50" placeholder="you@example.com" />
                </div>
                <div>
                  <label className="text-xs font-mono text-muted-foreground mb-1.5 block">{t('contact.message')}</label>
                  <Textarea value={form.message} onChange={e => setForm({ ...form, message: e.target.value })} className="bg-muted/30 border-border/50 focus:border-primary/50 min-h-[120px]" placeholder={t('contact.messagePlaceholder')} />
                </div>
...
<p className="text-sm text-muted-foreground">+998 90 594 95 37</p>
              </div>
            </div>
          </motion.div>
        </div>
